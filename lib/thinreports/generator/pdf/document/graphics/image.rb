  ...
  module Generator
    ...
    module PDF::Graphics
      ...
      def normalize_image_from_base64(image_type, base64_string)
        ...
        image_data = if image_type == 'image/png'
      ...
      def normalize_image_from_file(filename)
        ...
        if extname.downcase == '.png' || extname.empty?
          png_normalizer = PNGNormalizer.load_file(filename)

          if png_normalizer.need_normalize?
            image_id = filename
            image_data = png_normalizer.normalize
            return create_temp_imagefile(image_id, image_data)
          end
        end
        filename
      end
      ...
      def clean_temp_images
        temp_image_registry.each_value {|image_path| File.delete(image_path) }
      end
